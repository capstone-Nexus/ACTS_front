          ...
          },
          {
            ...
            onEvent: event => {
              ...
              if (event.type === 'content') {
                ...
                if (!id) return;
                setMessages(prev => prev.map(m => (m.id === id ? { ...m, content: m.content + event.content } : m)));
                return;
              }
              if (event.type === 'done') {
                return;
    ...
    <div className="w-full h-screen bg-white pt-[80px]">
      <div className="h-full flex">
          ...
          <div className="p-4 border-b border-[#E5E7EB]">
            ...
            >
              <div className="flex items-center gap-2 justify-center">
                <Image src={Icons.Write} alt="write" width={20} height={20} />
                <p className="text-[13px] font-medium">새 채팅</p>
              </div>
            </button>
          </div>

          <div className="flex-1 overflow-y-auto">
            {backendConfigured && chatList.length === 0 ? <p className="p-4 text-[13px] text-[#6B7280]">채팅 목록이 없습니다.</p> : null}

            {chatList.map(c => (
              <div key={c.idx} className={`w-full px-4 py-3 border-b border-[#F3F4F6] hover:bg-[#F9FAFB] transition-colors flex items-start gap-2 ${selectedChatIdx === c.idx ? 'bg-[#EEF6FF]' : 'bg-white'}`}>
                <button
                  onClick={() => {
                    streamAbortRef.current?.abort();
                ...
                >
                  {editingChatIdx === c.idx ? (
                    ...
                    <p className="text-[13px] font-semibold text-[#111827] truncate">{normalizeChatTitle(c.title)}</p>
                  )}
        ...
        <div className="flex-1 flex flex-col">
          <div className="h-[50px] flex items-center justify-between px-6">{error && <p className="text-[12px] text-red-600">{error}</p>}</div>

          <div className="flex-1 w-full overflow-y-auto px-6" ref={scrollRef}>
            <div className="max-w-[900px] mx-auto flex flex-col gap-[22px] py-6">{messages.length === 0 ? <MessageBubble sender="bot" content="저에게 무엇이든 물어보세요!" /> : messages.map(msg => <MessageBubble key={msg.id} sender={msg.sender} content={msg.content} />)}</div>
          </div>
          ...
          <div className="w-full flex justify-center px-6 pb-6 bg-white border-t border-[#E5E7EB]">
            <div className="mt-5 w-full max-w-[900px] h-[60px] bg-[#F5F5F5] border border-[#D2D2D2] rounded-[60px] p-[10px] flex items-center justify-between">
              ...
              />
              <button className={`w-[45px] h-[45px] rounded-full flex items-center justify-center transition-colors flex-shrink-0 ${isStreaming ? 'bg-gray-300 cursor-not-allowed' : 'bg-[#4A8AEE] hover:bg-[#3A7ADE]'}`} onClick={() => void sendMessage()} disabled={isStreaming}>
                <Image src={Icons.Up} alt="up" width={22} height={22} />
              </button>
            </div>
          </div>
        </div>
